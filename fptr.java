...
public class RecuOnCanvas extends JFrame {
	...
	this.beautify(depth); // Je nach Tiefe kolorieren
		...
		Point2D topPoint = new Point2D(midPointBottom.getX(), midPointBottom.getY() - (int) Math.round(line.getEuclideanLength() * 1));
		...
		if (depth < 8) {
			drawRecursive(new Line2D(leftLine.getMidPoint(), rightLine.getMidPoint()), depth + 1);
